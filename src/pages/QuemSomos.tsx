...
import { getPresidentes, getMembros, getEstrutura } from "@/lib/contentStorage";
...
const QuemSomos = () => {
  const presidentes = getPresidentes().sort((a, b) => a.ordem - b.ordem);
  const membros = getMembros().sort((a, b) => a.ordem - b.ordem);
  const estrutura = getEstrutura().sort((a, b) => a.ordem - b.ordem);

  const topLevel = estrutura.filter((e) => e.nivel < 3);
  const bottomLevel = estrutura.filter((e) => e.nivel === 3);
  ...
  return (
    <PageLayout>
      ...
      <section className="py-20 bg-background">
        <div className="container px-4 max-w-5xl mx-auto">
          ...
          <div className="flex flex-col items-center gap-4">
            {topLevel.map((item, i) => (
              <div key={item.id}>
                {i > 0 && <div className="w-px h-8 bg-primary/30 mx-auto" />}
                <div
                  className={`rounded-xl px-8 py-4 font-bold text-center ${
                    item.nivel === 0 || item.nivel === 2
                      ? "bg-primary text-primary-foreground"
                      : "bg-secondary text-secondary-foreground"
                  } ${item.nivel === 2 ? "text-lg" : ""}`}
              ...
              <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {bottomLevel.map((item) => (
                    <div key={item.id} className="bg-card rounded-xl px-6 py-3 font-semibold text-foreground text-center border border-border text-sm" style={{ boxShadow: "var(--card-shadow)" }}>
                      {item.titulo}
                    </div>
                  ))}
                </div>
      ...
      <section className="py-20 bg-card">
        <div className="container px-4 max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {membros.map((m, i) => (
              <motion.div key={m.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-background rounded-2xl p-6 border border-border text-center hover:border-primary/30 transition-all" style={{ boxShadow: "var(--card-shadow)" }}>
      ...
      {/* Histórias dos Presidentes */}
      {presidentes.length > 0 && (
        <section className="py-20 bg-background">
          <div className="container px-4 max-w-5xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
              <h2 className="text-2xl md:text-3xl font-extrabold text-foreground mb-4">Histórias dos Presidentes</h2>
              <p className="text-muted-foreground max-w-2xl mx-auto">Conheça os líderes que marcaram a trajetória do CMPCD Jaú e suas contribuições para a defesa dos direitos das pessoas com deficiência.</p>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {presidentes.map((p, i) => (
                <motion.div
                  ...
                  className="bg-card rounded-2xl border border-border overflow-hidden hover:border-primary/30 transition-all flex flex-col"
                  style={{ boxShadow: "var(--card-shadow)" }}
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <h4 className="text-lg font-bold text-foreground mb-2 line-clamp-1">{p.nome}</h4>
                    <p className="text-sm text-muted-foreground leading-relaxed line-clamp-4 flex-1">{p.descricao}</p>
                  </div>
                </motion.div>
              ))}
            </div>
